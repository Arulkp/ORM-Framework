...
class OrmSql:
    ...
    def select_all_columns(cls, tbname=None):
        return "SELECT column_name FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = 'public' AND table_name = '{x}';".format(
            x=tbname)
    ...
    def _search_flds_where_(cls, tbname=None, colist=[], filters=[]):
        init_q = "SELECT {fields} FROM {tbname} WHERE {conditon}".format(fields=", ".join(colist), tbname=tbname,
                                                                         conditon="{condition}")
